...
const AppHeader = ({ studentName = "Rohan M", avatarUrl }: AppHeaderProps) => {
  return (
    <header className="sticky top-0 z-40 w-full border-b bg-card">
      <div className="flex h-16 items-center justify-between px-6">
        {/* Logo */}
        <Link to="/modules" className="flex items-center gap-2 hover:opacity-80 transition-opacity cursor-pointer">
        ...
        <div className="flex-1 flex justify-center">
          <div className="flex items-center gap-3 px-4 py-2 rounded-full bg-muted">
            ...
            <span className="text-sm text-muted-foreground">Listening for commands...</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link to="/profile">
            <Button variant="ghost" className="gap-3 px-4 hover:bg-muted">
              <div className="w-8 h-8 rounded-full bg-muted flex items-center justify-center overflow-hidden border-2 border-primary">
                {avatarUrl ? (
                  <img src={avatarUrl} alt={studentName} className="w-full h-full object-cover" />
                ) : (
                  <span className="text-sm font-semibold">{studentName.charAt(0)}</span>
                )}
              </div>
